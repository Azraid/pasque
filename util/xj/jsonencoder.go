...
// An jsonEncoder writes JSON objects to an output stream.
...
func (enc *jsonEncoder) Encode(root *node) error {
	if enc.err != nil {
		return enc.err
	}
	if root == nil {
...
func (enc *jsonEncoder) format(n *node, lvl int) error {
	...
	if n.isComplex() {
		...
		if len(n.Data) > 0 {
			stag += fmt.Sprintf("\"%scontent\":%s,", contentPrefix, sanitiseString(n.Data))
		}
...
func (enc *jsonEncoder) write(s string) {
	enc.w.Write([]byte(s))
}
